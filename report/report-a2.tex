...
Observability Patterns e Event Sourcing applicato a un servizio) sono stati scelti \textbf{CQRS} e \textbf{Service Discovery}. La scelta non \`e
...
\paragraph{Come sono raccolte.}
\label{sec:metriche-gateway}
Per i tre servizi di dominio la strumentazione non \`e inserita nei
...
\begin{itemize}
  \item i metodi \texttt{processXxx()} validano la richiesta, \textbf{non modificano lo stato} e
        restituiscono l'evento da applicare; sono gli unici che possono fallire;
...
Tutte e sei le transizioni emettono ora un evento (\texttt{DeliveryScheduled},
...
\begin{itemize}
  ...
  \item \textbf{infrastructure}: \texttt{DeliveryEventStoreTest}, \texttt{DeliverySnapshotTest} e
        ...
        ordinamento, rifiuto delle scritture concorrenti, replay, snapshot e ricostruzione della
        vista) usando gli oggetti di dominio reali. Sono test \emph{sociable}.
...
Oltre ai quattro livelli, i tre servizi di dominio (\texttt{order-service},
\texttt{delivery-service} e \texttt{drone-service}) includono un \texttt{ArchitectureTest} basato su
ArchUnit, che verifica che il dominio non dipenda da application e infrastructure, che le porte
risiedano nell'application layer e che gli adapter risiedano nell'infrastructure layer. Non appartiene
alla test pyramid: \`e una \emph{fitness function}, cio\`e un controllo automatico di una
caratteristica architetturale, e come tale uno strumento di governance dell'architettura.

L'\texttt{api-gateway} ne \`e privo, per una ragione che vale la pena esplicitare: non avendo un core
di dominio da proteggere (\`e un componente di sola infrastruttura, come discusso nella
sezione~\ref{sec:metriche-gateway} a proposito delle metriche), la regola di dipendenza che le
fitness function verificano negli altri servizi non ha qui un equivalente.
...
risposta \`e la stessa del QAS-A, cio\`e \texttt{SCHEDULED} con \texttt{droneId} nullo, ma la causa \`e
